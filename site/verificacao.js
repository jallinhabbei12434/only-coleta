...
// === CRONÔMETRO DE EXPIRAÇÃO DO CÓDIGO ===
(function iniciarTimerExpiracao(segundosTotais = 240) {
  const spanTimer = document.getElementById('expirationTimer');
  if (!spanTimer) return;          // se o span não existir, aborta

  let resto = segundosTotais;      // 4 min = 240 s

  const formatar = s => {
    const m = Math.floor(s / 60);
    const sRestantes = s % 60;
    return `${m}m ${sRestantes.toString().padStart(2, '0')}s`;
  };

  spanTimer.textContent = formatar(resto);
  ...
  const id = setInterval(() => {
    ...
    if (resto <= 0) {
      clearInterval(id);
      spanTimer.textContent = '0m 00s';
      // aqui você pode, por exemplo, desabilitar o form ou exibir alerta
