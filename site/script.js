...
function mostrarPopupEspera() {
  ...
  if (!waitModal || !countdown) {
    console.warn("waitModal ou waitCountdown não encontrado!");
    return;
  }
  ...
  // Ativa modal de espera logo no clique
  mostrarPopupEspera();
  ...
  try {
    ...
    if (data.disponibilidade === "lotado") {
      showCustomAlert("Acessos gratuitos esgostados. Solicite gratuitamente via WhatsApp.");
